...
> No Shiplight API token found. Run `codex mcp login shiplight-cloud` to authenticate, or set the `SHIPLIGHT_API_TOKEN` environment variable. Then retry this command.

### Saving the Token After OAuth

After the user authenticates via `codex mcp login shiplight-cloud`, the MCP server provides a `get_api_token` tool. Call it to retrieve the token, then save it locally:
...
2. Tell the user: "Your API token has expired. Run `codex mcp login shiplight-cloud` to re-authenticate."
...
|-------------|----------------------|-------------------------------------|
| 401         | Invalid/expired token | Delete `~/.shiplight/credentials.json`, prompt user to run `codex mcp login shiplight-cloud` |
